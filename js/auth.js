  ...
  try {
    ...
    if (snap.exists()) {
      ...
      if (s.whatsappEnabled !== false && waNum && !document.getElementById("za-wa-float") && !location.pathname.endsWith("admin.html")) {
        ...
        // Mobile: bottom-right. Desktop: bottom-left — keeps it clear of the
        // product page's Add to Cart buttons and sticky bar CTA on the right.
        const waSide = window.matchMedia("(max-width: 768px)").matches ? "right" : "left";
        wa.style.cssText = `position:fixed;${waSide}:1rem;bottom:1.1rem;z-index:9990;width:46px;height:46px;border-radius:50%;background:#25D366;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 14px rgba(37,211,102,.45);transition:transform .15s;`;
